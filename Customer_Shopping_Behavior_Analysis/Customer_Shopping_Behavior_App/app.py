...
selected_subscription = st.sidebar.selectbox(
    "Subscription Status", ["All"] + sorted(df["subscription_status"].unique().tolist())
)
...
selected_shipping = st.sidebar.multiselect(
    "Shipping Type", sorted(df["shipping_type"].unique().tolist()),
    default=sorted(df["shipping_type"].unique().tolist())
)
...
with tab2:
    ...
    c1.plotly_chart(fig7, width="stretch")

    repeat_sub = (
        filtered[filtered["previous_purchases"] > 5]
        .groupby("subscription_status")["customer_id"].count().reset_index()
    )
    ...
    c2.plotly_chart(fig8, width="stretch")
    ...
    c3.plotly_chart(fig9, width="stretch")
    ...
    c4.plotly_chart(fig10, width="stretch")
    ...
    c5.plotly_chart(fig11, width="stretch")
    ...
    c6.plotly_chart(fig12, width="stretch")
...
with tab3:
    ...
    top_rated = (
        ...
        .mean().reset_index()
        .sort_values("review_rating", ascending=False).head(10)
    )
    ...
    c1.plotly_chart(fig13, width="stretch")

    # discount rate — using transform instead of apply to avoid pandas 3 warning
    disc_yes = filtered[filtered["discount_applied"] == "Yes"].groupby("item_purchased")["customer_id"].count()
    disc_total = filtered.groupby("item_purchased")["customer_id"].count()
    discount_rate = (disc_yes / disc_total * 100).round(2).reset_index()
    ...
    c2.plotly_chart(fig14, width="stretch")
...
with tab4:
    ...
    with st.expander("Q2 — Discount Users Who Spent Above Average"):
        ...
        q2 = filtered[
            (filtered["discount_applied"] == "Yes") &
            (filtered["purchase_amount_usd"] >= avg_purchase)
        ][["customer_id", "purchase_amount_usd"]].copy()
        q2.columns = ["Customer ID", "Purchase Amount (USD)"]
        st.write(f"Average purchase amount: **${avg_purchase:.2f}**")
        st.dataframe(q2.reset_index(drop=True), width="stretch")

    with st.expander("Q3 — Top 5 Products by Avg Review Rating"):
        q3 = (
            filtered.groupby("item_purchased")["review_rating"]
            .mean().round(2).reset_index()
            .sort_values("review_rating", ascending=False).head(5)
        )
        q3.columns = ["Product", "Avg Review Rating"]
        st.dataframe(q3.reset_index(drop=True), width="stretch")
    ...
    with st.expander("Q6 — Top 5 Products by Discount Usage Rate"):
        disc_yes2 = filtered[filtered["discount_applied"] == "Yes"].groupby("item_purchased")["customer_id"].count()
        disc_total2 = filtered.groupby("item_purchased")["customer_id"].count()
        q6 = (disc_yes2 / disc_total2 * 100).round(2).reset_index()
        q6.columns = ["Product", "Discount Rate (%)"]
        q6 = q6.sort_values("Discount Rate (%)", ascending=False).head(5).reset_index(drop=True)
        st.dataframe(q6, width="stretch")
    ...
    with st.expander("Q8 — Top 3 Products per Category"):
        ...
        q8["Rank"] = q8.groupby("Category")["Total Orders"].rank(
            method="first", ascending=False
        ).astype(int)
        q8 = q8[q8["Rank"] <= 3].sort_values(["Category", "Rank"]).reset_index(drop=True)
        st.dataframe(q8, width="stretch")
    ...
    with st.expander("Q10 — Revenue Contribution by Age Group"):
        ...
        st.dataframe(q10.reset_index(drop=True), width="stretch")
